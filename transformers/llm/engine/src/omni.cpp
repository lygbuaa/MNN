...
std::vector<int> Omni::qwen2VisionProcess(VARP image, bool new_image) {
    ...
    {
        HANG_STOPWATCH();
        RLOGW("Omni::visionProcess generate new imageEmbedding");
        imageEmbedding = mVisionModule->onForward(moduleInputs)[0];
        image_embedding_cache = imageEmbedding;
    }
    else
    {
        RLOGW("Omni::visionProcess using image_embedding_cache");
        imageEmbedding = image_embedding_cache;
    }
...
std::vector<int> Omni::visionProcess(const std::string& file, bool new_image) {
    ...
    if (inputNames.size() >= 3 && inputNames[0] == "patches") {
        imgIds = qwen2VisionProcess(image, new_image);
